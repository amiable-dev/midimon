...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("{}", "╔══════════════════════════════════════╗".cyan().bold());
    println!("{}", "║      MIDI Diagnostic Tool            ║".cyan().bold());
    println!("{}", "╚══════════════════════════════════════╝".cyan().bold());
    println!();
    ...
    let _conn = midi_in.connect(port, "diagnostic", move |_stamp, msg, _| {
        let now = Instant::now();
        ...
        *count += 1;
        ...
        match msg[0] & 0xF0 {
            ...
            0xE0 => {
                ...
                let direction = if centered > 0 { "↑" } else if centered < 0 { "↓" } else { "◯" };
                println!(
                    "{} value={:5} ({:+6}) ch={:2} {}",
    ...
    loop {
        ...
        if !held.is_empty() {
            let held_list: Vec<String> = held.iter()
                .map(|(note, press_time)| {
