...
use midimon_daemon::action_executor::{ActionExecutor, TriggerContext};
...
async fn test_event_processor_midi_to_processed() {
    ...
    match &processed_events[0] {
        midimon_core::event_processor::ProcessedEvent::PadPressed { note, velocity, .. } => {
...
async fn test_mapping_engine_event_to_action() {
    ...
    let config = Config {
        device: midimon_core::config::types::DeviceConfig {
            name: "Test Device".to_string(),
            auto_connect: false,
            auto_reconnect: false,
            port: None,
        },
        modes: vec![midimon_core::config::types::Mode {
            name: "Test Mode".to_string(),
            color: Some("blue".to_string()),
            mappings: vec![midimon_core::config::types::Mapping {
                trigger: midimon_core::config::types::Trigger::Note {
                    note: 60,
                    velocity_min: None,
                },
                action: midimon_core::config::types::ActionConfig::Keystroke {
    ...
    match action.unwrap() {
        midimon_core::actions::Action::Keystroke { keys, .. } => {
...
async fn test_velocity_context_propagation() {
    ...
    let action = midimon_core::actions::Action::Delay(1);
    ...
    let config = Config {
        ...
        modes: vec![midimon_core::config::types::Mode {
            ...
            mappings: vec![midimon_core::config::types::Mapping {
                ...
                action: midimon_core::config::types::ActionConfig::Delay { ms: 1 },
                description: None,
            }],
        }],
        global_mappings: vec![],
