...
async fn test_plugin_data_directory_created() {
    ...
    assert!(plugin_data_dir.exists(), "Plugin data directory should be created: {:?}", plugin_data_dir);
...
async fn test_filesystem_capability_not_granted() {
    ...
    assert!(config.capabilities.is_empty(), "Default config should have no capabilities");

    // Plugin should still load (but won't have filesystem access)
    let result = WasmPlugin::load(&wasm_path, config).await;
    assert!(result.is_ok(), "Plugin without filesystem capability should still load");
}
...
async fn test_multiple_plugins_share_data_directory() {
    ...
    let _plugin1 = WasmPlugin::load(&wasm_path, config1).await
    ...
    let _plugin2 = WasmPlugin::load(&wasm_path, config2).await
    ...
    assert!(plugin_data_dir.exists(), "Shared plugin data directory should exist");
}
...
async fn test_directory_permissions() {
    ...
    let metadata = std::fs::metadata(&plugin_data_dir)
...
async fn test_system_utils_with_filesystem_access() {
    ...
    assert!(result.is_ok(), "System utils plugin should work with filesystem access");
}
...
async fn test_config_with_multiple_capabilities() {
    ...
    let result = WasmPlugin::load(&wasm_path, config).await;
    assert!(result.is_ok(), "Plugin with multiple capabilities should load successfully");
}
