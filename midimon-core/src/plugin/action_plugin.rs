...
///
/// ```rust,no_run
/// use midimon_core::plugin::{ActionPlugin, TriggerContext, Capability};
/// use serde_json::Value;
/// use std::error::Error;
///
/// struct EchoPlugin;
...
pub trait ActionPlugin: Send + Sync {
    ...
    ///
    /// ```rust,no_run
    /// # use midimon_core::plugin::{ActionPlugin, TriggerContext};
    /// # use serde_json::Value;
    /// # use std::error::Error;
    /// # struct MyPlugin;
    ...
    ///
    /// ```rust,no_run
    /// # use midimon_core::plugin::{ActionPlugin, Capability};
    /// # struct HttpPlugin;
    /// # impl ActionPlugin for HttpPlugin {
    /// #   fn name(&self) -> &str { "http" }
    /// #   fn version(&self) -> &str { "1.0.0" }
    /// #   fn description(&self) -> &str { "HTTP plugin" }
    /// #   fn execute(&mut self, _: serde_json::Value, _: midimon_core::plugin::TriggerContext) -> Result<(), Box<dyn std::error::Error>> { Ok(()) }
